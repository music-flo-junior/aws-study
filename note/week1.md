...
- 과금 1시간 단위로 이뤄지며, 1분을 사용했더라도 1시간으로 책정
- 3가지 방식 중 요금이 가장 비쌈.
- 공유 인스턴스(Shared tenancy) : 하나의 물리적인 서버에 여러 개의 EC2 인스턴스가 실행. 다른 인스턴스가 서버 자원을 많이 소모한다면 현재 인스턴스의 성능에 영향이 있을 수 있음.
- 전용 인스턴스(Dedicated tenancy) : 하나의 물리적인  서버에 하나의 EC2 인스턴스가 실행. 서버 내에 다른 인스턴스가 없으므로 성능에 영향을 미치지 않으며, 공유 인스턴스 방식보다 비쌈.

> 스팟 인스턴스(Spot Instance)
- 경매 방식의 인스턴스
- 인스턴스의 스펙을 설정하고 원하는 가격을 입력하여 입찰하면 수요와 공급에 따라 가격이 결정되고 높게 입찰한 사람한테 인스턴스가 할당.
- 다음과 같은 상황에 적합
    - 클러스터링으로 이루어지는 분석 작업
    - 이미지 변환, 비디오 렌더링
    - 웹 크롤링
    - 각종 테스트
    - 기타 불시에 중단되어도 상관없는 경우

> 예약 인스턴스(Reserved Instance)
- 일정한 예약금을 선불로 내면 인스턴스를 1년 또는 3년동안 예약할 수 있음.
- Light / Medium / Heavy 사용률 예약 인스턴스가 존재함.


- 4.5 가상 스토리지를 제공하는 EBS
    - EC2 인스턴스에 장착하여 사용할 수 있는 가상 저장 장치.
    - EBS는 EC2 인스턴스에서 제공하는 기본 용량보다 더 사용해야 할때, 운영체제를 중단시키지 않고 용량을 자유롭게 늘리고 싶을 때, 영구적인 데이터 보관이 필요할 때, RAID등의 고급 기능이 필요할 때 사용.
    - EC2에 설치된 OS에 그냥 일반적인 하드 디스크나 SSD처럼 인식되며, 원하는 크기로 만들 수 있고 성능(IOPS) 또한, 원하는 수치로 설정 가능함.
    - 사용자가 삭제하기 전까지는 데이터가 안전하게 유지.
    - EBS의 Block은 블록 장치라고 하여, Unix/Linux 계열 OS에서 일정한 크기 단위로 읽고 쓰는 저장 장치를 부르는 말.


- EBS 기본 개념
    - 볼륨(Volume) : EBS의 가장 기본적인 형태로 OS에서 바로 사용 가능한 형태
    - 이미지(Image) : AMI를 줄여 부르는 말. OS가 설치된 형태이며 이 AMI로 EC2 인스턴스를 생성함.
    - 스냅샷(Snapshot) : EBS 볼륨의 특정 시점을 그대로 복사해 저장한 파일
    - IOPS : 저장 장치의 성능 측정 단위
    - 볼륨 포맷 : sudo mkfs -t ext4 /dev/sdf
    - 볼륨 마운트 : sudo mount /dev/sdf /mnt
    - 볼륨 제거 : sudo umount /mnt


- 4.8 EC2 기타 설정 및 기능
    - Placement Group : 물리적으로 인접한 곳에 EC2 인스턴스를 생성하여 네트워크 퍼포먼스를 극대화
    - Bundle Instance : 인스턴스 스토리지를 Root 장치로 사용하는 EC2 인스턴스의 내용을 S3 버킷에 저장하는 기능
    - Bundle Tasks : Bundle Instance 기능이 처리되는 과정을 Bundle Task
    - Network Interfaces : ENI를 생성하고 EC2 인스턴스에 장착
- 